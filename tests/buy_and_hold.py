"""
This is a script to test the portfolioManagement update code. We just test a simple buy and hold strategy and make sure it matches the return
...
from portfolioManagement.portfolioManagement import portfolioManager
...
def main():
    ...
    manager = portfolioManager(len(currencies))
    manager.portfolio[0] = 0.5
    manager.portfolio[1] = 0.5
    ...
    #plot predicted amount
    axs[0].plot(datetimes, [p[0] * 0.5 + p[1] * 0.5 for p in prices])
    plt.legend() 
    plt.show()
